...
| 優先する価格 | 買取価格 |
| 永続化 | self-hostのPostgreSQL 18。原本storageはDBと分離 |
...
├── compose.yaml                # API、Worker、DB、artifact storageのローカル構成
├── Dockerfile                  # APIとWorkerが共有するapplication image
├── .env.example                # ローカル設定の雛形。秘密値は置かない
├── docker/                     # container image向けの補助script
