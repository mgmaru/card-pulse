...
- [x] `CP-0092` `done` — 依存が落ちてもserviceが生き続ける性質をCIの回帰検査にする。
  - Depends on: `CP-0061`
...
- [x] `CP-0086` `done` — ADR管理の機械検査を導入するか再判断する。
  ...
  - Evidence: 導入した。`.agents/skills/write-project-docs/scripts/validate_adrs.py`が節と順序、ヘッダ5項目と順序、状態値、日付形式、置換関係の相互参照と置換された側の状態、一覧表との一致、`###`の位置、タイトル番号とファイル名の一致を検査する。CIの`Documentation` jobへ追加した。必須status checkに指定済みのjobへ置いたのは、新しいjobだと必須指定を足すまで失敗を無視してマージできるためである（`CP-0080`と同じ理由）。現行23件が通ることを確認したうえで、13通りの違反を一時的なcopyへ作って全件が検知されることを確認した。うち2件は今日`CP-0096`で修正した実在の不整合（ADR-0003の`後継ADR`、ADR-0001のヘッダ3項目欠落）の再現である。読みやすさの指針は検査対象に含めていない。一文が長くても明快な場合があり、機械が一律に却下すると書き手の判断を奪うためで、この線引きと次の再検討条件を[ADR管理の機械検査](../adr/README.md#adr管理の機械検査)へ記載した。実行条件は`AGENTS.md`と[`write-project-docs`](../../.agents/skills/write-project-docs/SKILL.md)へ追加した。
