...
| エンティティ | 役割 | 主な情報 |
| --- | --- | --- |
| `source` | 取得経路と現在の運用状態を識別する | 安定slug、種別、運用状態、停止理由、直近成功run、設定参照 |
...
- 予定された実行の欠落と、通信、接続先、アクセス拒否、応答不一致、構造変更、データ品質、保存の失敗段階を区別する。
- 異常な実行ではsourceの最終成功日時と観測の鮮度を更新せず、停止理由と診断に使ったrunまたはartifactを追跡できるようにする。
- sourceの停止、復旧確認、定期取得への再開について、状態、理由、実行者、日時、根拠を追跡できるようにする。
...
- parser再解析結果の置換・派生関係
- sourceの運用状態、状態変更履歴、直近試行・最終成功runを表す具体的なschema
- source固有の件数・解析率の基準値と、そのversion・変更理由を表す方法
