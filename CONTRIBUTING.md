...
pull requestは、変更内容をプロジェクトオーナーが読むための場です。文書だけの変更も例外にしません。

このリポジトリでは実装の多くをAIエージェントが行うため、pull requestが人間が差分を確認する唯一の地点になります。またADRや契約文書は、後続の実装すべてを拘束するという意味でコードより影響が長く残るため、機械検査を通るかどうかとは別に読む価値があります。

変更の種類によってpull requestの要否を変える案（散文は直接push、実行されるファイルはpull request）も検討しましたが、採用しませんでした。GitHubのrulesetは適用対象をref名でしか絞れず、パスによる出し分けを強制できないため、規約頼みになるためです。一律にすることで、判定を都度行う必要も無くなります。

pull requestを作る側は、読まれることを前提に本文を書きます。判断が分かれた点、採用しなかった選択肢、確信の低い箇所を明示し、1つのpull requestに1つのタスクだけを含めます。

auto-mergeは使いません。CIの通過と同時にマージされると、確認する機会が無くなるためです。
...
- ADRには判断理由を明示し、決め手となった一文を太字で強調します。詳しくは [判断理由の書き方](docs/adr/README.md#判断理由の書き方) を参照してください。
